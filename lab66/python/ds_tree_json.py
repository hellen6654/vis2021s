...
# datacsv = pd.read_csv(str(pathlib.Path.cwd())+'/cars.csv')
# datacsv = datacsv.fillna(0)
# datacsv = datacsv.set_index('brand')
# datacsv = datacsv.drop('Europe.')
# datacsv = datacsv.reset_index()
# labelencoder = LabelEncoder()
# datacsv['brand']= labelencoder.fit_transform(datacsv['brand'])


# datacsv = pd.read_csv(str(pathlib.Path.cwd())+'/healthcare-dataset-stroke-data.csv',keep_default_na=False)
# datacsv = datacsv.drop(labels=['id'],axis=1)
# datacsv = datacsv.drop(labels=['bmi'],axis=1)
# # datacsv = datacsv.set_index('bmi')
# # datacsv = datacsv.drop('N/A')
# datacsv = datacsv.set_index('gender')
# datacsv = datacsv.drop('Other')

# datacsv = datacsv.loc[(datacsv['age'] >= 65) & (datacsv['age'] < 85)]

# datacsv = datacsv.reset_index()
# labelencoder = LabelEncoder()
# datacsv['ever_married']= labelencoder.fit_transform(datacsv['ever_married'])
# datacsv['work_type']= labelencoder.fit_transform(datacsv['work_type'])
# datacsv['Residence_type']= labelencoder.fit_transform(datacsv['Residence_type'])
# datacsv['smoking_status']= labelencoder.fit_transform(datacsv['smoking_status'])
# datacsv['gender'] = labelencoder.fit_transform(datacsv['gender'])


#datacsv = pd.read_csv(str(pathlib.Path.cwd())+'/Breast_cancer_data.csv')
datacsv = pd.read_csv(str(pathlib.Path.cwd())+'/winequalityN.csv')
datacsv = datacsv.dropna()
labelencoder = LabelEncoder()
datacsv['type']= labelencoder.fit_transform(datacsv['type'])
red = (datacsv[datacsv['type'] == 0]).sample(n=250)
white = datacsv[datacsv['type'] == 1].sample(n=250)
datacsv = red.append(white)
columns = datacsv.columns.tolist()

#columns = columns[-1:] + columns[:-1]
#datacsv = datacsv[columns]
